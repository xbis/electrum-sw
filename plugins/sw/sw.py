...
try:
    import hid
    from btchip.btchipComm import DongleSmartcard, DongleWait
...
class Sw_Client():
    ...
    def __init__(self, cardReader):
        cardConnection = cardReader.createConnection()
        cardConnection.connect()
        self.dongleObject = DongleSmartcard(cardConnection)
    ...
    def test_pin_unlocked(func):
        ...
        def catch_exception(self, *args, **kwargs):
            try:
                ...
                if e.sw == 0x6982:
                    raise Exception(_('Your wallet is locked. Please unlock it.'))
                else:
                    raise
...
class Sw_KeyStore(Hardware_KeyStore):
    hw_type = 'sw'
    device = 'SW'
    ...
    def sign_transaction(self, tx, password):
        ...
        try:
            ...
            if segwitTransaction:
                app.startUntrustedTransaction(True, inputIndex,
                ...
                while inputIndex < len(inputs):
                    ...
                    app.startUntrustedTransaction(False, 0,
                                                            singleInput, redeemScripts[inputIndex])
                    inputSignature = app.untrustedHashSign(inputsPaths[inputIndex], pin, lockTime=tx.locktime)
...
class SwPlugin(HW_PluginBase):
    libraries_available = BTCHIP
    keystore_class = Sw_KeyStore
    client = None
    DEVICE_IDS = ('SW', ) # dummy
    ...
    def get_btchip_device(self, device):
        ...
        # return HIDDongleHIDAPI(dev, sw, BTCHIP_DEBUG)
        return None
